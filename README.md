# Abaco Coding Harness

Abaco Coding Harness is a **plugin-based agent factory and harness**. You assemble an agent from a **model**, a **channel**, and **plugins**, then run it from a CLI, a localhost HTTP factory, or the browser SPA.
...
Package name: `universal`. Product name: **Abaco Coding Harness**. The console script, imports, and `UNIVERSAL_*` env vars stay `universal`.
...
**Mac:** download **only** `Abaco-Coding-Harness.dmg` from [Releases](https://github.com/anthony-x507/Abaco-Universal-harnes-/releases). Open it and drag **Abaco Coding Harness.app** to `/Applications`. That is the official install. Source zips are not a product install and cannot self-update.
...
| `universal update` | Check GitHub Releases for a newer `Abaco-Coding-Harness.dmg`. `--apply` on the packaged Mac app. |
...
| `identity` | `show_identity`, `list_capabilities` | Report who the agent is (Abaco Coding Harness Agent) and list its capabilities. Canonical source `universal/identity.py`. Not a template; no `mother.yaml`. |
...
```bash
scripts/build_macos.sh    # bun build + PyInstaller → Abaco Coding Harness.app
scripts/create_dmg.sh     # Abaco-Coding-Harness.dmg (hdiutil)
```
...
The crystal Ábaco mark (`web/src/assets/logo.png`) is the window watermark (15% opacity), the header lockup next to **Abaco Coding Harness**, and the splash shown while the SPA mounts. `scripts/make_icns.sh` builds `Universal.icns` for the Dock, Finder, and title-bar icon; `build_macos.sh` packages that file into `Abaco Coding Harness.app`.
...
Self-update (packaged Mac app only): the repo `anthony-x507/Abaco-Universal-harnes-` is baked into `version.json`. On launch the SPA checks silently and prompts if a newer `.dmg` exists. Settings → **Check for Updates**. **Download now** replaces `/Applications/Abaco Coding Harness.app` and relaunches. Nothing is overwritten until you confirm. If the app is not in `/Applications`, Settings warns that updates will fail. Gatekeeper override is required until signing exists.

### Stuck on 1.2.15 / 1.2.16 — Mounted image has no Universal.app

v1.2.17 renamed the Mac bundle to **Abaco Harness.app** and the DMG volume to `Abaco Harness`. Clients still on **1.2.15 / 1.2.16** look only for `/Volumes/Universal/Universal.app` and fail with **Mounted image has no Universal.app**. From **1.2.18** the DMG is a bridge: it still includes a `Universal.app` copy of the same app on volume `Universal`, so those old updaters can apply.

**One-time manual install** if Check for Updates still fails after `v1.2.18` is tagged:

1. Quit Universal / Abaco Harness.
2. Download **only** `Abaco-Coding-Harness.dmg` from [Releases](https://github.com/anthony-x507/Abaco-Universal-harnes-/releases).
3. Open the DMG (the volume name is still `Universal` so old updaters can find it).
4. Drag **Abaco Coding Harness.app** to `/Applications` (not Downloads).
5. Optionally delete `/Applications/Universal.app` and `/Applications/Abaco Harness.app`.
6. Open Spotlight, type Abaco Coding Harness, confirm the path is `/Applications/Abaco Coding Harness.app`.
7. Header must say **Abaco Coding Harness**.

After a 1.2.15 auto-update applies this release, Finder may still show `Universal.app` until you drag the canonical bundle once or a later update relocates it. The header will already say Abaco Coding Harness. Python package `universal` and `UNIVERSAL_*` env vars stay unchanged.

GitHub Releases always serves the **same** `Abaco-Coding-Harness.dmg` to every computer. Two Macs do not share Application Support. On **one** Mac, WKWebView can keep a cached `http://127.0.0.1:43124/` from v1.0.0 (Universal Platform + Create General) even after you replace the `.app`. From 1.2.4 the window loads `/?v=<version>` and stores WebView data under Application Support. Incident notes for the designer: [docs/designer_incident_report.md](docs/designer_incident_report.md). To wipe **this** Mac (deletes chats and saved keys on that Mac only):

```bash
bash scripts/wipe_macos.sh
```

Then download [Abaco-Coding-Harness.dmg](https://github.com/anthony-x507/Abaco-Universal-harnes-/releases) for the current tag, drag **Abaco Coding Harness.app** to `/Applications` only, and open it from there. The header must say **Abaco Coding Harness**.
...
A tag `v*` on GitHub runs `.github/workflows/release.yml` (macOS runner with desktop + media extras) and attaches `Abaco-Coding-Harness.dmg`.
