#!/usr/bin/env bash
# Create Abaco-Coding-Harness.dmg from Abaco Coding Harness.app (macOS only for the image).
#
...
#   1. Copies Abaco Coding Harness.app (canonical; drag this to /Applications)
#   2. Copies the same tree as Universal.app (old updater path)
#   3. Uses -volname Universal so attach lands at /Volumes/Universal
# New updaters (1.2.18+) search Universal / Abaco Coding Harness / Abaco Harness
# volumes and all three bundle names.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"

APP_BUNDLE="${UNIVERSAL_APP_BUNDLE:-Abaco Coding Harness.app}"
CANONICAL_BUNDLE="Abaco Coding Harness.app"
LEGACY_BUNDLE="Universal.app"
DMG_NAME="Abaco-Coding-Harness.dmg"
...
STAGE="$(mktemp -d "${TMPDIR:-/tmp}/abaco-coding-harness-dmg.XXXXXX")"
cleanup() { rm -rf "$STAGE"; }
trap cleanup EXIT
