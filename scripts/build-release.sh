...
if [[ -f scripts/create_dmg.sh ]]; then
  ...
  echo "create_dmg.sh missing — packager still writes Abaco-Coding-Harness.dmg on a Mac runner"
fi

if command -v codesign >/dev/null 2>&1 && [[ -d "Abaco Coding Harness.app" ]]; then
  codesign -vvv "Abaco Coding Harness.app" || true
else
