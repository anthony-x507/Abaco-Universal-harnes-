...
- Product locks still win: package `universal`, product name Abaco Harness, never Aegis as a product name, no ChatGPT-shaped UI (the face/app is built from zero), one injected `AgentRegistry` + `AgentLifecycle`.
...
| `60.md` | Six polish / intelligence adjustments before Hito 4 | accepted — applied lock-safe |
...
| `68.md` | Designer stop: native `.dmg` vs self-update that cannot run | answered in 69 |
| `69.md` | Designer sí: baked repo, launch prompt, Settings, /Applications, retire zip | accepted — shipping |
...
- Notes still say Aegis / `factory/` / `aegis-agent/`. Product lock: Abaco Harness, package `universal/`.
