...
Browser SPA for Abaco Coding Harness. Pages: Chat, Agents, Design, Settings.
...
Open `http://127.0.0.1:43123`. Tab title: **Abaco Coding Harness**.

After `bun run build`, `universal serve` also serves `web/dist` from the factory origin (`/`). `universal desktop` opens that same URL in a native window. On a Mac, `scripts/build_macos.sh` builds `Abaco Coding Harness.app`.
